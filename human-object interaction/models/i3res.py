...
class I3ResNet(torch.nn.Module):
    def __init__(self, resnet2d, frame_nb=16, class_nb=1000, conv_class=False):
        ...
        self.conv1 = inflate.inflate_conv(
            resnet2d.conv1, time_dim=5, time_padding=2, time_stride=1)
        self.bn1 = inflate.inflate_batch_norm(resnet2d.bn1)
        self.relu = torch.nn.ReLU(inplace=True)
        self.maxpool1 = inflate.inflate_pool(
        ...
        if conv_class:
            ...
            final_time_dim = int(math.ceil(frame_nb / 1))
            self.avgpool = inflate.inflate_pool(
                resnet2d.avgpool, time_dim=final_time_dim, time_output_size=final_time_dim)
            self.fc = resnet2d.fc
...
class Bottleneck3d(torch.nn.Module):
    def __init__(self, bottleneck2d):
        ...
        self.conv1 = inflate.inflate_conv(
            bottleneck2d.conv1, time_dim=3, time_padding=1, time_stride=1)
        ...
        if bottleneck2d.downsample is not None:
            self.downsample = inflate_downsample(bottleneck2d.downsample, time_stride=1)
        else:
